// Toastify OAST - Core Cloudflare Worker Entrypoint
...
export default {
  // -------------------------------------------------------------
  // HTTP Fetch Handler: API, Dashboard, DoH & HTTP OAST Capture
  // -------------------------------------------------------------
  async fetch(request, env, ctx) {
    ...
    // 1. CORS Preflight Handling with strict Origin reflection
    ...
    // 3. DNS-over-HTTPS (DoH) API Endpoint (RFC 8484 / JSON DoH)
    if (pathname === '/dns-query') {
      return handleDoHRequest(request, env, url);
    }

    // 4. Determine if request is an OAST Callback vs Dashboard Access
    const payloadId = extractPayloadId(url, hostname);

    if (payloadId) {
      // It's an incoming OAST HTTP/HTTPS interaction!
      return recordAndRespondOastHttp(request, env, payloadId, url);
    }

    // 5. Default to Static UI Assets / Dashboard
    if (env.ASSETS) {
      return env.ASSETS.fetch(request);
    }

    return new Response('Toastify OAST Server Active.', { status: 200 });
  },

  // -------------------------------------------------------------
  // Cloudflare Email Routing Handler (SMTP Interactions)
  // -------------------------------------------------------------
  async email(message, env, ctx) {
    try {
      ...
      // Extract payload ID from recipient address (e.g. "c94a2b1f@oast.mydomain.com")
...
function extractPayloadId(url, hostname) {
  // Case A: Subdomain format e.g. "c94a2b1f.oast.mydomain.com" or "c94a2b1f.sub.workers.dev"
  ...
  // Case B: Explicit URL Path `/p/c94a2b1f` or `/payload/c94a2b1f`
  const pathParts = url.pathname.split('/');
  if ((pathParts[1] === 'p' || pathParts[1] === 'payload') && pathParts[2]) {
    return pathParts[2].toLowerCase();
  }

  // Case C: Query Parameter `?oast=c94a2b1f` or `?id=c94a2b1f`
...
async function recordAndRespondOastHttp(request, env, payloadId, url) {
  ...
  // Check for custom Mock Response in database
